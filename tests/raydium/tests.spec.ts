...
import { AddLiquidityInfo, CreatePoolInfo, RemoveLiquidityInfo, SwapInfo } from 'parser/raydium/v4/types';
...
describe('Raydium Parser', () => {
    ...
    test('parse should correctly identify swap action [base in]', async () => {
        ...
        for (const action of result?.actions || []) {
            ...
            expect(action.info.poolId.toString()).toEqual('ZFBZunJyh7HZWJrrUUazZiVegN8SXXBrHYVYMEeWG4T')
            expect((action.info as SwapInfo).tokenIn.toString()).toEqual("So11111111111111111111111111111111111111112")
            expect((action.info as SwapInfo).tokenOut.toString()).toEqual("GPrF7LXiQAY8Y9Fci7et2C7a9JsrCBDRvEAKLCjLpump")
    ...
    test('parse should correctly identify swap action [base out]', async () => {
        ...
        for (const action of result?.actions || []) {
            expect(action.type).toEqual("swap")
            expect(action.info.poolId.toString()).toEqual('43QDFSth4Fu2JFVKto13zASQu8Dc7RL6wkUqksgeYvqq')
            expect((action.info as SwapInfo).tokenIn.toString()).toEqual("9Zw3CR7NPD6hXNk5PZYpYKsvWv9puwr1eEaPxXRapump")
            expect((action.info as SwapInfo).tokenOut.toString()).toEqual("So11111111111111111111111111111111111111112")
            expect((action.info as SwapInfo).tokenInDecimal).toEqual(BigInt(9))
    ...
    test('parse create pool info', async () => {
        ...
        for (const action of result?.actions || []) {
            ...
            expect(action.info.poolId.toString()).toEqual('43QDFSth4Fu2JFVKto13zASQu8Dc7RL6wkUqksgeYvqq')
            expect((action.info as CreatePoolInfo).baseMint.toString()).toEqual("9Zw3CR7NPD6hXNk5PZYpYKsvWv9puwr1eEaPxXRapump")
            expect((action.info as CreatePoolInfo).quoteMint.toString()).toEqual("So11111111111111111111111111111111111111112")
            expect((action.info as CreatePoolInfo).baseDecimals).toEqual(9)
            expect((action.info as CreatePoolInfo).quoteDecimals).toEqual(9)
            expect((action.info as CreatePoolInfo).baseAmountIn).toEqual(BigInt("4000000000000000"))
            expect((action.info as CreatePoolInfo).quoteAmountIn).toEqual(BigInt("100000000"))
            expect((action.info as CreatePoolInfo).marketId.toString()).toEqual("Cux4RjDHhRUYkNLePxrRo97ppg3LopHfg1mE2RF2gPY")
            expect((action.info as CreatePoolInfo).user.toString()).toEqual("5z4giZ7YjS7LMGYfPjCUA8qUUxNSFm3Ru3xGtKCziqb6")
        }
    });

    test('parse add liquidity action', async () => {
        //         const sig = "pXPLUDJPBn58aYkP9d91fySxpiy8qzFmwid33jPsMKG2TfkasT7zMTAu15HuBBQ6NFLXHpM27yx8ooCikzjjrEg"
        // const txn = await connection.getParsedTransaction(sig, { maxSupportedTransactionVersion: 0})
        // fs.writeFileSync('tests/raydium/parsed-deposit-txn.json', JSON.stringify(txn, null, 2))
        ...
        for (const action of result?.actions || []) {
            ...
            expect(action.info.poolId.toString()).toEqual('43QDFSth4Fu2JFVKto13zASQu8Dc7RL6wkUqksgeYvqq')
            expect((action.info as AddLiquidityInfo).baseMint.toString()).toEqual("9Zw3CR7NPD6hXNk5PZYpYKsvWv9puwr1eEaPxXRapump")
            expect((action.info as AddLiquidityInfo).quoteMint.toString()).toEqual("So11111111111111111111111111111111111111112")
            ...
            expect((action.info as AddLiquidityInfo).baseAmountIn).toEqual(BigInt("214323699689057"))
            ...
            expect((action.info as AddLiquidityInfo).user.toString()).toEqual("5z4giZ7YjS7LMGYfPjCUA8qUUxNSFm3Ru3xGtKCziqb6")
        }
    });
    ...
    test('parse remove liquidity action', async () => {
        ...
        for (const action of result?.actions || []) {
            ...
            expect(action.info.poolId.toString()).toEqual('43QDFSth4Fu2JFVKto13zASQu8Dc7RL6wkUqksgeYvqq')
            expect((action.info as RemoveLiquidityInfo).baseMint.toString()).toEqual("9Zw3CR7NPD6hXNk5PZYpYKsvWv9puwr1eEaPxXRapump")
            expect((action.info as RemoveLiquidityInfo).quoteMint.toString()).toEqual("So11111111111111111111111111111111111111112")
            ...
            expect((action.info as RemoveLiquidityInfo).baseAmountOut).toEqual(BigInt("590358717042758"))
            ...
            expect((action.info as RemoveLiquidityInfo).user.toString()).toEqual("5z4giZ7YjS7LMGYfPjCUA8qUUxNSFm3Ru3xGtKCziqb6")
        }
    });

    test('parseMultiple should parse multiple transactions', async () => {
        const result = await parser.parseMultiple([swapBaseInTransaction, swapBaseOutTransaction, createPoolTransaction, addLiquidityTransaction, removeLiquidityTransaction])
